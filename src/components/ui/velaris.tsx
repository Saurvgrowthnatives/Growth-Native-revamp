...
const Velaris = ({
  ...
  const [renderMode, setRenderMode] = useState<"pending" | "webgl" | "fallback">(
    "pending"
  );
  ...
  const fallbackGradient =
    renderMode === "fallback"
      ? `linear-gradient(115deg, ${bg} 0%, ${colors[2] ?? colors[0]} 32%, ${
          colors[1] ?? colors[0]
        } 55%, ${colors[0]} 75%, ${bg} 100%)`
      : undefined;
